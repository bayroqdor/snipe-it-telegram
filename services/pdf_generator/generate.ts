...
async function drawTable(page: any, element: TableElement, fonts: Map<string, any>): Promise<void> {
  ...
  const defaultRowHeight = element.rowHeight || 20;
  ...
  for (let rowIndex = 0; rowIndex < element.rows.length; rowIndex++) {
    ...
    const rowHeight = (element.rowHeights && element.rowHeights[rowIndex]) ? element.rowHeights[rowIndex] : defaultRowHeight;
